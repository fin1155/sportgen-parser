...
shiny::testServer(app_env$server, {
  ...
  session$setInputs(table_columns=c('mirna','cell_line','title'), column_draft=c('title','disease'))
  verify(identical(names(visible_result_data()),c('mirna','cell_line','title')))
  verify(length(selected_columns()) == 3L)
  session$setInputs(table_columns=character())
  verify(identical(selected_columns(), TABLE_COLUMN_PRESETS$core))
  session$setInputs(table_columns=c('title','mirna'))
  session$setInputs(export_filtered=TRUE,results_view=list(version=table_version(),rows=list()))
  verify(nrow(export_data()) == 0L)
  session$setInputs(results_view=list(version=table_version()-1L,rows=list()))
  verify(nrow(export_data()) == 1L)
  session$setInputs(results_view=list(version=table_version(),rows=list(1L)),export_filtered=FALSE)
  verify(nrow(export_data()) == 1L)
  session$setInputs(research_profile='general',query_pubmed='',run=6)
  verify(run_state()$kind == 'error')
  verify(length(calls) == 2L)
  session$setInputs(query_pubmed='miR-21 AND cancer',run=7)
  verify(run_state()$kind == 'success')
  verify(length(calls) == 3L)
})
unlink(archive)
